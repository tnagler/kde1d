...
//! @return bin counts
inline Eigen::VectorXd linbin(const Eigen::VectorXd &x, double a, double b,
                              size_t m, const Eigen::VectorXd &weights) {
...
double bkfe(const Eigen::VectorXd &x, size_t drv, double h, double a,
            double b) {
  size_t m = x.size();
...
inline double dpik(const Eigen::VectorXd &x,
  ...
  try {
    ...
    double alpha =
        std::pow(std::pow(2.0, 11.0 / 2.0) / (7.0 * effn), 1.0 / 9.0);
    double psi6hat = bkfe(x2, 6, alpha, sa, sb);
    alpha =
        std::pow(-3.0 * std::sqrt(2.0 / M_PI) / (psi6hat * effn), 1.0 / 7.0);
